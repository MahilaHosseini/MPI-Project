...
OUTPUT_FILE="./results/mpi_multi_results_summary4.txt"
mkdir -p ./results
...
mpirun -np 2 -hostfile $HOSTFILE $BINARY 512 >> $OUTPUT_FILE
...
echo "Running with np=4:" >> $OUTPUT_FILE
    ...
    # Run the MPI program and append its output to the file
mpirun -np 4 -hostfile $HOSTFILE $BINARY 1024 >> $OUTPUT_FILE
...
echo "Running with np=8:" >> $OUTPUT_FILE
    ...
    # Run the MPI program and append its output to the file
mpirun -np 8 -hostfile $HOSTFILE $BINARY 2048 >> $OUTPUT_FILE
...
echo "Running with np=16:" >> $OUTPUT_FILE
    ...
    # Run the MPI program and append its output to the file
mpirun -np 16 -hostfile $HOSTFILE $BINARY 4096 >> $OUTPUT_FILE
